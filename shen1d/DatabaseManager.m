...
#define Database_File_Name @"greenBerryReader.db"
...
- (NSArray *)queryMyshopsFromTable
{
	...
	NSString *sSQL = [[NSString alloc] initWithFormat:SQL_QUERY_MYSHOPS_INFO_FROM_TABLE,TableNameMyShops];
	FMResultSet *rs = [m_db executeQuery:sSQL];
    //	SAFE_RELEASE(sSQL);
	NSMutableArray *arrMyShops = [[NSMutableArray alloc] init];
	while ([rs next])
	{
		NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:
                             [rs stringForColumn:@"shopId"],@"shopId",
                             [rs stringForColumn:@"cardType"],@"cardType",
                             [rs stringForColumn:@"shopName"],@"shopName",                             
                             [rs dataForColumn:@"images"],@"images",nil];
//        NSLog(@"%@",[rs stringForColumn:@"userId"]);
		[arrMyShops addObject:dic];
        //		[dic release];
	}
	// Close database
	[m_db close];
    ...
    return (NSArray *)arrMyShops;
}
- (void)insertMyShops:(NSArray *)arrMyShops
{
    ...
    BOOL bAlreadyExist;
    NSString *sSQL = nil;
    for (NSUInteger i = 0; i < [arrMyShops count]; i++) {
        NSDictionary *dic = [arrMyShops objectAtIndex:i];
        bAlreadyExist = [self queryDataIfExist:[dic objectForKey:@"shopId"] tableName:TableNameMyShops];
        if (YES == bAlreadyExist)
        {
            LOGS(@"Metadata:%@ already exists in the table:%@", [dic objectForKey:@"shopId"], TableNameMyShops);
            return;
        }
        if (![m_db open])
        {
        ...
        sSQL = [[NSString alloc] initWithFormat:SQL_INSERT_MYSHOPS_INFO_TABLE,
                TableNameMyShops,
                @"shopId,cardType,shopName,images"];
        bAlreadyExist = [m_db executeUpdate:sSQL,
                         [dic objectForKey:@"shopId"],
                         [dic objectForKey:@"cardType"],
                         [dic objectForKey:@"shopName"],
                         [dic objectForKey:@"images"]];
        NSLog(@"insert:%i",bAlreadyExist);
        //        SAFE_RELEASE(sSQL);
        // Close database
        [m_db close];
    }
...
- (NSArray *)queryPersonalInfoFromTable
{
	// Open database
	if (![m_db open])
	{
	...
	NSString *sSQL = [[NSString alloc] initWithFormat:SQL_QUERY_PERSONAL_INFO_FROM_TABLE,TableNamePersonalInfo];
	FMResultSet *rs = [m_db executeQuery:sSQL];
//	SAFE_RELEASE(sSQL);
	NSMutableArray *arrPersonalInfo = [[NSMutableArray alloc] init];
	while ([rs next])
	{
		NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:
                             [rs stringForColumn:@"userId"],@"userId",
                             [rs stringForColumn:@"email"],@"email",
                             [rs stringForColumn:@"password"],@"password",
                             [rs stringForColumn:@"uname"],@"uname",
                             [rs stringForColumn:@"phone"],@"phone",
                             [rs stringForColumn:@"created"],@"created",
                             [rs dataForColumn:@"qrcode"],@"qrcode",nil];
        NSLog(@"%@",[rs stringForColumn:@"userId"]);
		[arrPersonalInfo addObject:dic];
//		[dic release];
	}
	// Close database
	[m_db close];
...
//	LOGS(@"queryDataFromTable:%@",[[arrBookData objectAtIndex:0] objectAtIndex:0]);
//	return (NSArray *)[arrBookData autorelease];
    return (NSArray *)arrPersonalInfo;
}

- (void)insertPersonalInfo:(NSDictionary *)dicPersonalInfo
{

    BOOL bAlreadyExist;
    NSString *sSQL = nil;
    bAlreadyExist = [self queryDataIfExist:[dicPersonalInfo objectForKey:@"userId"] tableName:TableNamePersonalInfo];
    if (YES == bAlreadyExist)
    {
        LOGS(@"Metadata:%@ already exists in the table:%@", [dicPersonalInfo objectForKey:@"userId"], TableNamePersonalInfo);
        return;
    }
    if (![m_db open])
    {
    ...
    sSQL = [[NSString alloc] initWithFormat:SQL_INSERT_PERSONAL_INFO_TABLE,
                      TableNamePersonalInfo,
                      @"userId,email,password,uname,phone,created,qrcode"];
    bAlreadyExist = [m_db executeUpdate:sSQL,
                     [dicPersonalInfo objectForKey:@"userId"],
                     [dicPersonalInfo objectForKey:@"email"],
                     [dicPersonalInfo objectForKey:@"password"],
                     [dicPersonalInfo objectForKey:@"uname"],
                     [dicPersonalInfo objectForKey:@"phone"],
                     [dicPersonalInfo objectForKey:@"created"],
                     [dicPersonalInfo objectForKey:@"qrcode"]];
    NSLog(@"insert:%i",bAlreadyExist);
//        SAFE_RELEASE(sSQL);
    // Close database
    [m_db close];
...
- (BOOL)queryDataIfExist:(NSString *)userId tableName:(NSString*)tableName
{
	BOOL bRet = NO;
	// Open database
	if (![m_db open])
	...
	NSString *sSQL = [[NSString alloc] initWithFormat:SQL_QUERY_DATA_IF_EXIST, tableName, @"userId"];
	FMResultSet *rs = [m_db executeQuery:sSQL, userId];
//	SAFE_RELEASE(sSQL);
	while ([rs next])
	{
		NSString * m_userId = [rs stringForColumn:@"userId"];
		if ([m_userId isEqualToString:userId])
		{
			bRet = YES;
			break;
		}
